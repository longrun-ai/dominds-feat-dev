# Spec: File Edit Tools Unified Redesign (Preview-first + Single Apply)

- Status: Implemented (breaking change; no compat)
- Date: 2026-01-24
...
（改造前）Dominds 曾存在三套“写文件”心智模型并存：
- 精确困难场景：`preview_file_modification` → `apply_file_modification`（有 diff + 证据，安全）
- 易场景：直接写入类工具（`append_file` / `insert_after` / `insert_before`；**已移除**）
- 块替换：`preview_block_replace` → 专用 apply（`apply_block_replace`；**已移除**）
...
- 统一所有文本写入为 **preview-first + preview diff + evidence**，让 agent “敢改、能复核”。
- 统一 apply：所有计划类编辑（append/insert/block/replace-range）都通过 **`apply_file_modification`** 落盘。
- 统一输出：preview/apply 的 YAML 字段结构趋同，减少 agent 解析成本。
...
- 不提供任何旧工具名兼容层（不做 alias、不过渡；旧工具直接不存在）。
...
- 要求：任何 preview→apply 必须是 **两步、两条消息**（或由 runtime/tooling 提供“顺序编排器”能力）。

> 未决：是否需要新增“顺序编排”能力（例如 `orchestrate`/`transaction`），让 agent 可在单条消息中安全串行 preview→apply→next preview。

## 5. 工具集合（Tool Surface）

### 5.1 保留并扩展：`preview_file_modification`
...
- `appended_line_count`
- `normalized`（至少）：
  - `file_eof_has_newline`
  - `content_eof_has_newline`
...
- 错误与歧义必须失败（planning 阶段总是 strict；不提供 `strict=false` 回退）
...
### 5.6 移除（Breaking change, no compat）
- `append_file`（用 `preview_file_append` + `apply_file_modification` 替代）
- `insert_after` / `insert_before`（用 `preview_insert_after` / `preview_insert_before` + apply 替代）
- `apply_block_replace`（被 `apply_file_modification` 替代）
- `replace_block`（用 `preview_block_replace` + apply 替代）
...
未决但必须明确的一点：当文件在 preview→apply 之间发生变化，apply 的拒绝应尽量“只在必要时发生”。
...
- 所有文本写入均可通过 `preview_*` 获得 diff + 证据 + hunk_id，并且 **唯一**使用 `apply_file_modification` 落盘。
- preview/apply 的 YAML 字段可稳定解析，summary 可扫读，且中英/中日不会混乱。
- 换行与空行风格在 preview 阶段可见（至少 warning），避免“粘行/双空行”惊喜。
- 锚点歧义永不静默写入：必须失败并提供 `candidates_count`/下一步建议。
- 竞态风险有明确 guidance：工具文案/文档强调 preview→apply 需分消息或使用编排器。

## 9. 回归清单（交 @qa 纳 gate）

- `preview_file_modification`：定位证据 + apply 证据齐全；并发修改后 `context_match=fuzz|rejected` 语义正确。
- `preview_file_append`：EOF 无换行/空文件/已有尾随空行等情况下，normalized 与提示字段正确。
- `preview_insert_after` / `preview_insert_before`：锚点不存在/多次出现/occurrence 越界/错误码稳定。
- `preview_block_replace`：unique/ambiguous/not found；apply 后内容正确；文件变化时拒绝语义合理。
- `apply_file_modification`：能 apply 来自所有 preview_* 的 hunk；过期/不存在 hunk 给出可行动错误。
...
1) apply 拒绝策略：是否允许“文件其它位置变更但目标块仍可唯一匹配”时继续 apply（建议：允许并标记 fuzz）
2) 空行风格：仅提示 warning 还是提供可选自动对齐（`normalize_blanklines=`）
3) hunk 生命周期：TTL 默认多久？是否允许跨进程重启持久化（当前看像内存型）
4) 是否需要顺序编排器：让 agent 在单条消息中串行 preview→apply（降低并发竞态）
