...
This is a practical checklist for team members (and contributors) to quickly build an accurate mental model of Dominds and produce shippable feature briefs with clear acceptance gates.
...
- Ownership / scope: which team member owns which change — read each member's `gofor` (when to find them) and `nogo` (when to route elsewhere) in `.minds/team.yaml`; cross-domain work defaults to `@fullstack` (which freezes contracts single-handedly)
...
- Regression checklist: add a small, runnable smoke list

## 4) Handoff readiness (gate-ready)
