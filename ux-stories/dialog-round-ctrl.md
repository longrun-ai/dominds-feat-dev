...
```text
Step 1: Call the function tool `clear_mind` with:
{ "reminder_content": "Remember to verify the new-round prompt." }
Issue exactly one tool call. No other tool calls or extra text.
```
...
const msgId = await fillAndSend(
  'Step 1: Call the function tool `clear_mind` with {\"reminder_content\":\"Remember to verify the new-round prompt.\"}. ' +
    'Issue exactly one tool call. No other tool calls or extra text.',
);
...
```text
Step 3 (optional): Call the function tool `clear_mind` with:
{}
Issue exactly one tool call. No other tool calls or extra text.
```
...
```text
Step 5C: Call the function tool `clear_mind` with:
{ "reminder_content": "Remember SUBDLG_TOKEN_V2" }
Issue exactly one tool call. No other tool calls or extra text.
```
...
const msgC = await fillAndSend(
  'Step 5C: Call the function tool `clear_mind` with {\"reminder_content\":\"Remember SUBDLG_TOKEN_V2\"}. ' +
    'Issue exactly one tool call. No other tool calls or extra text.',
);
let toolAppeared = false;
try {
...
```text
Retry: Call the function tool `clear_mind` with {} now. Issue exactly one tool call. No other tool calls or text.
```
...
```text
Teardown Step A: Call the function tool `delete_reminder` with:
{ "reminder_no": <reminderNoToDelete> }
Issue exactly one tool call. No other tool calls or extra text.
```
...
If you added multiple reminders, repeat `delete_reminder({ "reminder_no": <currentCount> })` one-by-one until you reach `baselineReminders`.
